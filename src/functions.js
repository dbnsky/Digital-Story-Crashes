...
function fillMap(selection, color, data) {
  ...
  function setPathTitle(selection, data) {
      selection
      .text(function(d) { return  d.properties.name + ", " + (typeof data[d.id] === 'undefined' ? 'N/A' : data[d.id]); });
  }
  ...
  function updateMap(data) {
    ...
    var mapcountry = data.reduce(function(map, obj) {
  ...
  function calcColorScale(data) {
      ...
      // get values and sort
      let data_values = Object.values(data).sort( function(a, b){ return a.value-b.value; });

      quantiles_calc = quantiles.map( function(elem) {
                      return Math.ceil(d3.quantile(data_values, elem));
      });
    
      console.log(quantiles_calc);
